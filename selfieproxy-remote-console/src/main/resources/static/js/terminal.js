(function () {
	...
	// load() is a Promise (server-persisted settings, see settings.js) -- everything below depends
	// on the resolved settings, so the whole session setup waits for it. load() never rejects (it
	// falls back to defaults on any error), so there's no failure path to handle here.
	window.SelfieProxyTerminalSettings.load().then(function (settings) {
		...
		window.SelfieProxyTerminalSettings.initPanel(terminal, settings, {
