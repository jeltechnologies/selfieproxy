...
func (h *WebUiHandler) handleWebUiRequest(w http.ResponseWriter, r *http.Request) {
	...
	if tokenData.Agent != "" {
		w.WriteHeader(403)
		h.alertDialog(w, r, "This token is limited to a specific agent and cannot be used for the web UI", "/")
		return
	}
	...
	case "/add-token-agent":
		...
		err := h.tmpl.ExecuteTemplate(w, "add_token_agent.tmpl", templateData)
		if err != nil {
			w.WriteHeader(500)
			io.WriteString(w, err.Error())
			return
...
func (h *WebUiHandler) handleAgents(w http.ResponseWriter, r *http.Request, user User, tokenData TokenData) {
	...
	case "GET":
		...
		for _, user := range users {
			for agentName, agent := range user.Agents {
		...
		err := h.tmpl.ExecuteTemplate(w, "agents.tmpl", templateData)
		if err != nil {
			w.WriteHeader(500)
			io.WriteString(w, err.Error())
			return
	...
	case "POST":
		...
		err := h.api.SetAgent(tokenData, r.Form, owner, agentName)
		if err != nil {
			w.WriteHeader(500)
			h.alertDialog(w, r, err.Error(), "/agents")
			return
		}
...
func (h *WebUiHandler) confirmDeleteAgent(w http.ResponseWriter, r *http.Request) {
	...
	data := &ConfirmData{
		Head:       h.headHtml,
		Message:    fmt.Sprintf("Are you sure you want to delete agent %s for user %s?", agentName, owner),
		ConfirmUrl: fmt.Sprintf("/delete-agent?owner=%s&agent-name=%s", owner, agentName),
		CancelUrl:  "/agents",
	}
...
func (h *WebUiHandler) deleteAgent(w http.ResponseWriter, r *http.Request, tokenData TokenData) {
	...
	err := h.api.DeleteAgent(tokenData, owner, agentName)
	if err != nil {
		w.WriteHeader(500)
		h.alertDialog(w, r, err.Error(), "/agents")
		return
