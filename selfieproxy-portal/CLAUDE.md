...
- **Port Forwarding**: up to 8 individual forwarded TCP ports -- never a range, each is its own
  ...
  **Description** (`PortForwardingConfig.description`) is a plain free-text field, entirely
  optional, purely for the admin's own benefit (eg. "Minecraft server") -- never validated, never
  used by tunnel creation/matching, capped only client-side (`maxlength="200"`) with no
  server-side length check. Included in configuration export/import like every other Server field
